...
async function likeComment(req, res) {
    ...
    try {
        ...
        if (article.author !== user.username) {
            return res.status(403).json({ message: "Forbidden: Only the article owner can like/unlike comments" });
        }
        ...
        return res.status(200).json({ message, comment: comments[commentIndex] });
    } catch (err) {
        console.error(err);
        return res.status(500).json({ message: "Server error" });
    }
